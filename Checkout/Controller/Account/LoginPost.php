...
class LoginPost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    */
    ...
    public function execute()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
					...
					if($customer && isset($requestData['redirectcheckout'])){
						if(isset($requestData['remember_me'])){
							$this->rememberMe->delete();
							$cookieArray = array('remember_me'=>$requestData['remember_me'],'customer'=>true,'email'=>$login['username'],'password'=>$login['password']);
                    ...
                    if (!$this->getScopeConfig()->getValue('customer/startup/redirect_dashboard') && $redirectUrl) {
                        ...
                        //to check if index value is coming or not
                        return $resultRedirect;
                    }
		...
		if(isset($requestData['redirectcheckout'])){
			$resultRedirect = $this->resultRedirectFactory->create();
			$resultRedirect->setPath('customer/account/signin');
			return $resultRedirect;
		}
